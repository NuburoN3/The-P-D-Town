    ...
    function drawNPCs(cam) {
      ...
      for (const npc of npcs) {
        ...
        if (
          ...
          if (npc.sprite) {
            if (npc.desiredHeightTiles && npc.sprite.width && npc.sprite.height) {
              const targetHeight = TILE * npc.desiredHeightTiles;
              const scale = targetHeight / npc.sprite.height;
              const drawWidth = npc.sprite.width * scale;
              const drawHeight = npc.sprite.height * scale;
              const drawX = Math.round(npc.x - cam.x - (drawWidth - TILE) / 2);
              const drawY = Math.round(npc.y - cam.y - (drawHeight - TILE));
              ...
              ctx.restore();
            } else if (npc.spriteWidth && npc.spriteHeight) {
              const drawWidth = npc.spriteWidth;
              const drawHeight = npc.spriteHeight;
              const drawX = Math.round(npc.x - cam.x - (drawWidth - TILE) / 2);
              const drawY = Math.round(npc.y - cam.y - (drawHeight - TILE));
              ...
              const drawWidth = TILE;
              const drawHeight = TILE;
              const drawX = Math.round(npc.x - cam.x);
              const drawY = Math.round(npc.y - cam.y);
